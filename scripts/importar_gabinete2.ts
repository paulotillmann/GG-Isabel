...
function parsePhones(raw: string | null | undefined): string[] {
  ...
  // Se for uma mensagem que não sabe o número, retorna vazio
  if (rawLower.includes('não sabia') || rawLower.includes('não sabe') || rawLower === 'ii') {
    return [];
  }
...
// Parseia a data de nascimento no formato DD/MM/YY para YYYY-MM-DD
function parseBirthDate(raw: string | null | undefined): string | null {
  if (!raw) return null;
  const clean = raw.trim();
  if (clean === '' || clean === 'II') return null;
...
async function run() {
  console.log('Iniciando processamento do CSV de Gabinete 2...');

  // 1. Carrega todos os telefones existentes do banco para desduplicação
  const existingPhones = new Set<string>();

  // Tenta ler do arquivo gerado pela query do MCP
  const outputQueryPath = path.resolve('C:/Users/user/.gemini/antigravity-ide/brain/d753cd40-71d6-4fa5-b61b-3c4a63f76f3d/.system_generated/steps/483/output.txt');
  
  if (fs.existsSync(outputQueryPath)) {
    try {
      console.log(`Lendo telefones existentes do arquivo de saída da query: ${outputQueryPath}`);
      const content = fs.readFileSync(outputQueryPath, 'utf8');
      
      // Encontra a lista JSON de telefones usando Regex
      const jsonMatch = content.match(/\[\s*\{\s*"phone"\s*:\s*"[^"]*"\s*\}.*?\]/s);
      if (jsonMatch) {
        ...
        console.log(`Carregados ${existingPhones.size} telefones existentes a partir do log do MCP.`);
      } else {
        console.warn('Regex não encontrou o array JSON no arquivo de log da query.');
      }
    } catch (err) {
      console.error('Erro ao ler ou parsear o arquivo de saída da query:', err);
    }
  }

  // Tenta ler também do db_phones.json para compatibilidade e histórico adicional
  ...
  if (fs.existsSync(dbPhonesPath)) {
    try {
      ...
      console.log(`Adicionados ${count} telefones adicionais a partir de db_phones.json.`);
    } catch (err) {
      console.error('Erro ao ler db_phones.json:', err);
    }
  }

  console.log(`Total de telefones únicos mapeados para desduplicação: ${existingPhones.size}`);

  // 2. Lê e processa o arquivo CSV
  const csvPath = path.resolve('importar/gabinte2_full_isabel.csv');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Colunas: full_name;phone;birth_date;address
    const columns = line.split(';');
    const rawName = columns[0] || '';
    const rawPhone = columns[1] || '';
    const rawBirthDate = columns[2] || '';
    const rawAddress = columns[3] || '';

    // REGRA 1: "remover nomes em branco" / "não importar nomes em branco"
    ...
    // REGRA 2: Processamento e split de telefones (phone e phone_extra)
    ...
    const extraPhone = phones.length > 1 ? phones[1] : null;

    // REGRA 3: "não importar phones duplicados ou em branco" (verificação do telefone principal)
    // Desduplicação contra o banco
    if (existingPhones.has(mainPhone)) {
      skippedDuplicateInDb++;
      continue;
    }

    // Desduplicação contra o próprio CSV nesta rodada
    ...
    if (addressFull && addressFull.toLowerCase() !== 'ii' && addressFull.toLowerCase() !== '----') {
    ...
    pessoasToInsert.push({
      ...
      destino: 'GABINETE 2', // REGRA: "definir para todas as importações: GABINETE 2"
      birth_date,
  ...
  // Salva no db_phones.json atualizado para futuras deduplicações
  ...
  for (let i = 0; i < pessoasToInsert.length; i += batchSize) {
    ...
    batch.forEach(p => {
      sql += `INSERT INTO public.pessoa (id, full_name, phone, phone_extra, address, address_number, neighborhood, city, person_type, atendimento_humano, destino, birth_date) VALUES ('${p.id}', ${escapeSql(p.full_name)}, ${escapeSql(p.phone)}, ${escapeSql(p.phone_extra)}, ${escapeSql(p.address)}, ${escapeSql(p.address_number)}, ${escapeSql(p.neighborhood)}, 'Araguari - MG', 'Pessoa', false, 'GABINETE 2', ${escapeSql(p.birth_date)}) ON CONFLICT (id) DO NOTHING;\n`;
    });

    sql += '\nCOMMIT;\n';

    const batchIndex = (i / batchSize) + 1;
    const outSqlPath = path.resolve(`scripts/importar_gabinete2_batch_${batchIndex}.sql`);
    fs.writeFileSync(outSqlPath, sql, 'utf8');
    generatedFiles.push(outSqlPath);
    console.log(`Lote ${batchIndex} gerado em: ${outSqlPath}`);
  }

  console.log(`\nTodos os ${generatedFiles.length} arquivos SQL em lotes de Gabinete 2 foram gerados com sucesso!`);
}
