...
async function run() {
  console.log('Iniciando processamento do CSV de Audiência Pública (audiencia publica - isabel.csv)...');
  ...
  const csvPath = path.resolve('importar/audiencia publica - isabel.csv');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Colunas: full_name;address;phone
    const columns = line.split(';');
    const rawName = columns[0] || '';
    const rawAddress = columns[1] || '';
    const rawPhone = columns[2] || '';

    // REGRA 1: "não importar nomes em branco" / marcadores inválidos ou 'Sem nome'
    ...
    if (phones.length > 0) {
      ...
      // Se não possui telefone, avalia se possui endereço válido
      ...
      if (!hasValidAddress) {
        // Não possui endereço válido além do nome: descarta!
        skippedBlankPhones++;
        continue;
      }
    ...
    pessoasToInsert.push({
      ...
      destino: 'AUDIENCIA PUBLICA PARQUE LINEAR', // REGRA: destino unificado
      birth_date: null,
  ...
  console.log(`Pessoas sem telefone descartadas (sem endereço): ${skippedBlankPhones}`);
  console.log(`Pessoas sem telefone importadas (possuem endereço): ${importedWithoutPhone}`);
