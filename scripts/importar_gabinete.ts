...
function parsePhones(raw: string | null | undefined): string[] {
  ...
  // Se for uma mensagem que não sabe o número ou sem contato, retorna vazio
...
async function run() {
  console.log('Iniciando processamento do CSV de Gabinete 1 (gabinete_isabel.csv)...');
  ...
  const csvPath = path.resolve('importar/gabinete_isabel.csv');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (phones.length > 0) {
      ...
      // Se não possui telefone, avalia se possui endereço válido ou data de nascimento válida
      const addressClean = rawAddress.trim().toLowerCase();
      const hasValidAddress = addressClean !== '' && 
                              addressClean !== 'ii' && 
                              addressClean !== '----' && 
                              addressClean !== 'sem contato';
      
      const hasValidBirthDate = birth_date !== null;

      if (!hasValidAddress && !hasValidBirthDate) {
        // Não possui nem endereço nem nascimento válidos além do nome: descarta!
        skippedBlankPhones++;
        continue;
      }
    ...
    if (addressFull && addressFull.toLowerCase() !== 'ii' && addressFull.toLowerCase() !== '----' && addressFull.toLowerCase() !== 'sem contato') {
    ...
    pessoasToInsert.push({
      ...
      destino: 'GABINETE', // REGRA: destino uniforme como 'GABINETE'
      birth_date,
  ...
  console.log(`Pessoas sem telefone descartadas (sem endereço ou nascimento): ${skippedBlankPhones}`);
  console.log(`Pessoas sem telefone importadas (possuem endereço ou nascimento): ${importedWithoutPhone}`);
