        ...
        >
          ...
          <div className="flex items-center gap-3 mb-5 shrink-0">
            <div className="h-11 w-11 rounded-xl bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center shrink-0">
              <DatabaseBackup className="h-6 w-6 text-purple-600 dark:text-purple-400" />
            </div>
            <div>
                ...
                {historico.map((log) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 text-sm">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2 min-w-0">
                        <div className="min-w-0 flex-1 break-words">
                          <span className="font-semibold text-slate-900 dark:text-white mr-2">
                            {log.acao === 'CRIADO' ? 'Novo' : 'Alteração em'}{' '}
                            {log.entidade_tipo}:
                          </span>
                          <span className="text-slate-700 dark:text-slate-300 font-medium break-all sm:break-words">
                      ...
                      {log.detalhes_alteracao && Object.keys(log.detalhes_alteracao).length > 0 && (
                        <div className="mt-2 space-y-1 bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 min-w-0">
                          {Object.entries(log.detalhes_alteracao).map(([campo, mudanca]: [string, any]) => (
                            <div key={campo} className="flex flex-col sm:flex-row sm:items-center gap-1 text-xs min-w-0">
                              <span className="font-semibold text-slate-600 dark:text-slate-400 capitalize w-24 shrink-0">
                                {campo}:
                              </span>
                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                {mudanca.antigo !== undefined && (
                                  <>
                                    <span className="text-red-500 dark:text-red-400 line-through truncate max-w-[120px] sm:max-w-[200px] min-w-0" title={String(mudanca.antigo)}>
                                      {String(mudanca.antigo) || '(vazio)'}
                                    </span>
                                    <span className="text-slate-400 text-[10px] shrink-0">➔</span>
                                  </>
                                )}
                                <span className="text-emerald-600 dark:text-emerald-400 font-medium truncate flex-1 min-w-0" title={String(mudanca.novo || mudanca)}>
