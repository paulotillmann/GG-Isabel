...
async function sendWhatsApp(phone: string, fullName: string, customMessage?: string) {
  ...
  let message = `Olá *${firstName}*, tudo bem?\n\nHoje é um dia muito especial! Em nome do Gabinete da Vereadora Isabel, gostaríamos de lhe desejar um **Feliz Aniversário**! 🎉🥳\n\nQue seu dia seja repleto de alegrias, saúde e paz. Um forte abraço!`;
...
async function processBirthdays(targetId?: string) {
  ...
  const isManualAll = targetId === 'all';
  const actualTargetId = isManualAll ? undefined : targetId;
  
  if (!isManualAll && !actualTargetId) {
  ...
  if (actualTargetId) {
    toProcess = toProcess.filter((p: any) => p.id === actualTargetId);
  }
