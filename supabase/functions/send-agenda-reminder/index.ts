...
const INSTANCE_NAME = Deno.env.get("EVOLUTION_INSTANCE_NAME") || "Dona Pires";
...
function buildReminderMessage(compromisso: any): string {
  ...
  lines.push(``, `_Lembrete automático do Gabinete da Vereadora Isabel_`);
...
async function processReminders() {
  ...
  // Horário atual BRT em formato HH:MM:SS (independente do timezone da máquina)
  const currentTimeStr = nowBRT.toISOString().slice(11, 19);
  ...
  const limitTimeStr = limitDate.toISOString().slice(11, 19);
  ...
  const { data: toProcess, error: queryError } = await query;

  if (queryError) {
    console.error("[Agenda Reminder] Error fetching agenda:", queryError);
    throw queryError;
  }
  ...
  // 2. Buscar usuários que devem receber lembretes (apenas administradores)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fallback 1: comparação direta com env var (compatibilidade)
    if (!isServiceRole) {
    ...
    // Fallback 2: comparação com o segredo estático do cron de GG-Isabel
    const cronSecret = "cron-secret-gg-isabel-2026";
    if (!isServiceRole && token === cronSecret) {
      isServiceRole = true;
      console.log("[Agenda Reminder] Authenticated via cron secret match");
    }
